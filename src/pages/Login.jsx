...
function Login() {
    ...
    const loginRequest = async () => {
        try {
            const response = await axios.post(
                "https://fcback.onrender.com/api/v1/users/login",
                user
            );
    ...
    return (
        <div className="bg-cover bg-center w-screen h-screen flex justify-center items-center" style={{ backgroundImage: `linear-gradient(rgba(0, 0, 0, 0.3), rgba(0, 0, 0, 0.5)), url(${backgroundImage})` }}>
            <style>
                {`
                    @font-face {
                        font-family: 'Gilroy';
            ...
            <h1 style={{ fontFamily: "Gilroy, sans-serif", textAlign: "center", fontSize: "30px", marginBottom: "20px", color: "green", userSelect: "none" }}> FARMER - LOGIN</h1>
                <div className="flex flex-col items-center">
                    ...
                    >
                        ...
                        <button
                            type="submit"
                            className="btn p-1 py-3 rounded border-2 bg-transparent text-black border-black hover:border-black hover:bg-green-800 hover:text-white duration-700 w-60"
                        >
                            Login
                        </button>
                    </form>
